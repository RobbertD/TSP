...
% load the data sets
datasetslist = dir('template_path/datasets/');
...
plot(x,y,'ko')
ah2 = axes('Parent',fh,'Position',[.65 .65 .3 .3]);
axes(ah2);
xlabel('Generation');
...
crossover_set = {'xalt_edges', 'order_high_level', 'pmx', 'scx'};
mutation_set ={'inversion', 'insert', 'scramble'};
...
llooppopup = uicontrol(ph,'Style','popupmenu','String',{'off','on'},'Value',1,'Position',[390 260 50 20],'Callback',@twooptpopup_Callback);
twooptpopup = uicontrol(ph,'Style','popupmenu','String',{'off','on'},'Value',1,'Position',[390 50 50 20],'Callback',@llooppopup_Callback);
twooptpopuptxt = uicontrol(ph,'Style','text','String','2-opt','Position',[310 50 50 20]);
...
crossover = uicontrol(ph,'Style','popupmenu', 'String', crossover_set, 'Value',1,'Position',[10 50 130 20],'Callback',@crossover_Callback);
mutation = uicontrol(ph,'Style','popupmenu', 'String',mutation_set, 'Value',1,'Position',[150 50 130 20],'Callback',@mutation_Callback);
...
set(fh,'Visible','on');
    ...
    function twooptpopup_Callback(hObject,eventdata)
        lloop_value = get(hObject,'Value');
        if lloop_value==1
            TWO_OPT = 0;
        else
    ...
    function runbutton_Callback(hObject,eventdata)
        ...
        run_ga_path_repr(x, y, NIND, MAXGEN, NVAR, ELITIST, STOP_PERCENTAGE, PR_CROSS, PR_MUT, CROSSOVER, MUTATION, LOCALLOOP,  TWO_OPT, ah1, ah2, ah3);
        end_run();
    end
